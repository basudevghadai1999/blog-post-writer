...
class BlogPostCreator:
    ...
    def get_links(self):
        try:
            ...
            wrapper = DuckDuckGoSearchAPIWrapper()
            search_results = wrapper.results(self.keyword, max_results=self.number_of_web_references)

            links = [result['link'] for result in search_results if 'link' in result]
            
            if not links:
                print("⚠️ No links found! Search might be blocked.")
                return []
    ...
    def create_blog_post(self):
        try:
            ...
            if not links:
                return "⚠️ No valid web references found. Try again later."
            ...
            if not docs:
                return "⚠️ No content loaded from web pages."
